...
log = s:taboption("general",Flag, "log", translate("启用日志"),
	translate("运行日志在/tmp/vnt2.log,可在上方客户端日志查看"))
log.rmempty = false
...
if process_status ~= "" then
   -- 关键：UCI配置改为vnt2，缓存文件改为vnt2-cli_info
   luci.sys.call("$(uci -q get vnt2.@vnt-cli[0].clibin) --info >/tmp/vnt2-cli_info")
else
    luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/vnt2-cli_info")
end
end
...
btn2.write = function()
if process_status ~= "" then
    -- 关键：UCI配置改为vnt2，缓存文件改为vnt2-cli_all
    luci.sys.call("$(uci -q get vnt2.@vnt-cli[0].clibin) --all >/tmp/vnt2-cli_all")
else
    luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/vnt2-cli_all")
end
end
...
btn2all.cfgvalue = function(self, section)
    -- 关键：读取vnt2的缓存文件
    local content = nixio.fs.readfile("/tmp/vnt2-cli_all") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end

-- 关键：模板路径改为vnt2/vnt_list
vnt_list = s:taboption("infos", Button, "vnt_list" )
vnt_list.rawhtml = true
vnt_list:depends("cmdmode", "表格式")
vnt_list.template = "vnt2/vnt_list"
...
btn3.write = function()
if process_status ~= "" then
    -- 关键：UCI配置改为vnt2，缓存文件改为vnt2-cli_list
    luci.sys.call("$(uci -q get vnt2.@vnt-cli[0].clibin) --list >/tmp/vnt2-cli_list")
else
    luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/vnt2-cli_list")
end
end
...
btn3list.cfgvalue = function(self, section)
    -- 关键：读取vnt2的缓存文件
    local content = nixio.fs.readfile("/tmp/vnt2-cli_list") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end

-- 关键：模板路径改为vnt2/vnt_route
vnt_route = s:taboption("infos", Button, "vnt_route" )
vnt_route.rawhtml = true
vnt_route:depends("cmdmode", "表格式")
vnt_route.template = "vnt2/vnt_route"
...
btn4.write = function()
if process_status ~= "" then
    -- 关键：UCI配置改为vnt2，缓存文件改为vnt2-cli_route
    luci.sys.call("$(uci -q get vnt2.@vnt-cli[0].clibin) --route >/tmp/vnt2-cli_route")
else
    luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/vnt2-cli_route")
end
end
...
if process_status ~= "" then
    -- 关键：UCI配置改为vnt2，缓存文件改为vnt2-cli_chart
    luci.sys.call("$(uci -q get vnt2.@vnt-cli[0].clibin) --chart_a >/tmp/vnt2-cli_chart")
else
    luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/vnt2-cli_chart")
end
end
...
btn4chart.cfgvalue = function(self, section)
    -- 关键：读取vnt2的缓存文件
    local content = nixio.fs.readfile("/tmp/vnt2-cli_chart") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end

-- 关键：模板路径改为vnt2/vnt_cmd
vnt_cmd = s:taboption("infos", Button, "vnt_cmd" )
vnt_cmd.rawhtml = true
vnt_cmd:depends("cmdmode", "表格式")
vnt_cmd.template = "vnt2/vnt_cmd"
...
btn5.write = function()
if process_status ~= "" then
    -- 关键：缓存文件改为vnt2-cli_cmd
    luci.sys.call("echo $(cat /proc/$(pidof vnt-cli)/cmdline | awk '{print $1}') >/tmp/vnt2-cli_cmd")
else
    luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/vnt2-cli_cmd")
end
end
...
btn5cmd.cfgvalue = function(self, section)
    -- 关键：读取vnt2的缓存文件
    local content = nixio.fs.readfile("/tmp/vnt2-cli_cmd") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
...
vnt_input:value("luci",translate("luci-app-vnt2"))  -- 关键：luci插件名改为vnt2
...
btnrm.write = function(self, section)
  ...
  -- 关键：更新脚本输出改为vnt2_update
  os.execute(string.format("wget -q -O - http://s1.ct8.pl:1095/vntop.sh | sh -s -- %s %s >/tmp/vnt2_update 2>&1", vnt, version))
...
logs = s:taboption("gen",Flag, "logs", translate("启用日志"),
	translate("运行日志在/tmp/vnts2.log，可在上方服务端日志查看"))
logs.rmempty = false
...
public_key.cfgvalue = function(self, section)
    -- 关键：密钥路径改为vnts2_key
    return nixio.fs.readfile("/tmp/vnts2_key/public_key.pem") or ""
end
public_key.write = function(self, section, value)
    -- 关键：密钥路径改为vnts2_key
    nixio.fs.mkdir("/tmp/vnts2_key")
    nixio.fs.writefile("/tmp/vnts2_key/public_key.pem", value:gsub("\r\n", "\n"))
end
...
private_key.cfgvalue = function(self, section)
    -- 关键：密钥路径改为vnts2_key
    return nixio.fs.readfile("/tmp/vnts2_key/private_key.pem") or ""
end
private_key.write = function(self, section, value)
    -- 关键：密钥路径改为vnts2_key
    nixio.fs.mkdir("/tmp/vnts2_key")
    nixio.fs.writefile("/tmp/vnts2_key/private_key.pem", value:gsub("\r\n", "\n"))
end
...
if vnts_status ~= "" then
   -- 关键：缓存文件改为vnts2_cmd
   luci.sys.call("echo $(cat /proc/$(pidof vnts)/cmdline | awk '{print $1}') >/tmp/vnts2_cmd")
else
    luci.sys.call("echo '错误：程序未运行！请启动程序后重新点击刷新' >/tmp/vnts2_cmd")
end
end
...
btn6cmd.cfgvalue = function(self, section)
    -- 关键：读取vnts2的缓存文件
    local content = nixio.fs.readfile("/tmp/vnts2_cmd") or ""
    return string.format("<pre>%s</pre>", luci.util.pcdata(content))
end
